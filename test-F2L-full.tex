...
\cs_new:Npn \FLL_wrapper:n #1 {
    \FLL_split:w #1 \q_stop
}

\cs_new:Npn \FLL_split:w #1 #2 #3 #4 \q_stop {
    \hbox{\FLL {#1} {#2} {#3} {#4}}
}
...
\begin{tcolorbox}[
    blankest,
    breakable,
    break at=11.2\gfllheight, % 11个公式一页高度（使用实际测量值）
...
\seq_map_function:NN \g_cube_fll_all_seq \FLL_wrapper:n
\ExplSyntaxOff
% \mbox{\hspace{\gfllwidth}\vspace{3\gfllheight}} % 最后留点空白
\end{tcolorbox}
...
\begin{tcolorbox}[blankest,height=11.3\gfllheight+2pt,width=\linewidth,halign=center]
    \begin{tcolorbox}[
        enhanced,
        colback=myblue,
        colframe=black!70,
